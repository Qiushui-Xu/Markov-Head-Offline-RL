...
class SequenceTrainer:
    def __init__(self, config: TrainerConfig, model: DecisionTransformer, offline_dataset, rollout_dataset = None, is_gym = False):
        ...
        if config.use_control:
            ...
            for name, param in model.named_parameters():
                if "alpha" in name:
                    ctl_para.append(param)
                else:
                    arch_para.append(param)

            self.arch_opt = torch.optim.AdamW(
                arch_para,
                lr=config.arch_lr,
                weight_decay=1e-4,
            )
            ...
            self.ctl_opt = torch.optim.AdamW(
                ctl_para,
                lr=config.ctl_lr,
                weight_decay=1e-4,
            )
    ...
    def _run_epoch(self, epoch_num):
        ...
        if self.config.use_control:
            trn_size = int(0.8 * len(dataset))
            tst_size = len(dataset) - trn_size
            trn_dataset, tst_dataset = torch.utils.data.random_split(dataset, [trn_size, tst_size])

            trn_loader = DataLoader(trn_dataset, shuffle=True, pin_memory=True,
                                batch_size= self.config.batch_size,
                                num_workers= self.config.num_workers)
            tst_loader = DataLoader(tst_dataset, shuffle=True, pin_memory=True,
                                batch_size= self.config.batch_size,
                                num_workers= self.config.num_workers)
        ...
        for it, (states, actions, _, rtgs, timesteps, attention_mask) in pbar:
            ...
            self.logger.logkv_mean("arch_loss", arch_loss.item())
            pbar.set_description(f"Epoch {epoch_num+1}, iter {it}: train arch_loss {arch_loss.item():.5f}.")

            if self.config.use_control and it % self.config.frec == 0:
                (states, actions, _, rtgs, timesteps, attention_mask) = next(tst_iter)
                states = states.type(torch.float32).to(self.device)
                actions = actions.type(torch.float32).to(self.device)
                rtgs = rtgs.type(torch.float32).to(self.device)
                timesteps = timesteps.to(self.device).long()
                ...
                ctl_loss.backward()
                self.ctl_opt.step()
                self.model.transformer.transformer.h[0].attn.show_alpha()
    ...
    def train(self):
        ...
        for epoch in range(self.config.max_epochs):
            ...
            if self.config.last_eval and epoch < self.config.max_epochs - 1:
                pass
            # elif self.config.model_prune is False:
            else:
